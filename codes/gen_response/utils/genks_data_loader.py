...
class GPTData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print(f"dialog id {example['dialog_id']}")
        # print(f"turn id {example['turn_id']}")
        # print(f"text {example['text']}")
        # print(f"context {example['context']}")
        # print(f"knowledge {example['knowledge']}")
        # print()
        # exit()
        ...
        sent_id += 1
        sequence_str += f'<s{id_map[sent_id]}>{sentence}<s{id_map[sent_id]}>\n'
        ...
        for pid, (title, passage) in enumerate(knowledge.items()):
            ...
            for sentence in passage:
                if len(sequence) > self.max_length:
                    break
                sent_id += 1
                sequence_str += f'<s{id_map[sent_id]}>{sentence}'
                ...
                sequence_str += f'<s{id_map[sent_id]}>\n'
