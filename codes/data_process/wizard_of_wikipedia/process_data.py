...
# 2-knowledge: 汇总当前turn的前两轮对应的retrieved passages以及当前对话相关的知识（chosen_topic_passage）到当前turn对应的knowledge库中

# 3-labels: 当前Wizard turn的句子

# 3-text: 当前turn对应的

# 第一个example：如果第一个句子是wizard，context即为topic；如果第二个句子是wizard，context为topic+apprentice

# 后面的example：ext是apprentice

# 4-label_cands：只对valid和test有效

# 5-dialogue context：当前wizard对应的前面的turn，包括：speaker，text，topic，checked sentence（if have）
...
class WizardDialogKnowledgeTeacher:
    ...
    def _format_example(self, episode_idx, entry_idx=0):
        d = self.raw_data[episode_idx] # get episode_idx-th episode
        episode_done = entry_idx == (self.len_episode(episode_idx) - 1) # Whether this turn is the last turn

        wizard_first = 'Wizard' in d['dialog'][0]['speaker'] # Whether the wizard is the first one
        print(f"{wizard_first}")
        idx = entry_idx * 2 if wizard_first else (entry_idx * 2) + 1 # echo example contains two turns as context,
        # make sure the last turn is the apprentice's turn and the current turn is the wizard's turn
        ...
        if not wizard_first or idx != 0: # first turn is apprentice (id is 1, 3, 5) or not the first turn
        ...
        # merge retrieved passages and topic pasagges
        knowledge_dict = {chosen_topic: chosen_topic_passages} # topic: passages
        ...
        if idx == 0:
            ...
            text += apprentice_entry['text'] # get context's last turn's text

        # next, get label
        wizard_entry = d['dialog'][idx] # current entry is wizard entry
...
def main():
    data_dir = r'/mnt/ai2lab/weishao4/programs/InterpretableKGC/data/wizard_of_wikipedia'
    os.system("mkdir -p /mnt/ai2lab/weishao4/programs/InterpretableKGC/data/wizard_of_wikipedia/processed_data")
    for fname in os.listdir(data_dir):
        if "train" in fname or "valid" in fname or "test" in fname:
            raw_data = json.load(open(f'/mnt/ai2lab/weishao4/programs/InterpretableKGC/data/wizard_of_wikipedia/{fname}'))
            agent = WizardDialogKnowledgeTeacher(raw_data, datatype='train' if "train" in fname else "test")
            total = 0
            num = 0
            num_b = 0
            turns = []
            ...
            print(len(saved))
            print(num)
            json.dump(saved, open(f'/mnt/ai2lab/weishao4/programs/InterpretableKGC/data/wizard_of_wikipedia/processed_data/{fname}', 'w'))
